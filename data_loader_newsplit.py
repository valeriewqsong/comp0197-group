...
def split_data(annotations_file1, annotations_file2):
    """Split data into train, validation and test sets in 0.7"""
    with open(annotations_file1, 'r') as f:
        img_labels1 = [tuple(line.strip().split(' ')[:2]) for line in f if line.strip() and line.strip().split(' ')[0]]
    with open(annotations_file2, 'r') as f:
        img_labels2 = [tuple(line.strip().split(' ')[:2]) for line in f if line.strip() and line.strip().split(' ')[0]]
    img_labels = img_labels1 + img_labels2
    
    np.random.shuffle(img_labels)
    dev_labels = img_labels[:int(0.8*len(img_labels))]
    test_labels = img_labels[int(0.8*len(img_labels)):]
    np.random.shuffle(dev_labels)
    train_labels = dev_labels[:int(0.7*len(dev_labels))]
    val_labels = dev_labels[int(0.7*len(dev_labels)):]
    return train_labels, val_labels, test_labels

def split_labels(train_file, split_ratio=11, test=False):
    ...
    labeled_samples = len(train_file) // split_ratio
    labeled_data = train_file[:labeled_samples]
    unlabeled_data = train_file[labeled_samples:]
...
def get_data_loader(basedir = "./", batch_size=32, split_ratio = 10, num_workers=0):
    ...
    train_labels, val_labels, test_labels = split_data(os.path.join(basedir, 'annotations/trainval.txt'), os.path.join(basedir,'annotations/test.txt'))
    labeled_data, unlabeled_data = split_labels(train_labels, split_ratio=split_ratio)
    print("labeled training data:", len(labeled_data))
    print("unlabeled training datadata:", len(unlabeled_data))
    print("validation data:", len(val_labels))
    print("test data:", len(test_labels))
    ...
    val_labeled_dataset = OxfordPetsDataset(os.path.join(basedir,'images'), img_labels=val_labels, transform=data_transforms)
    test_labeled_dataset = OxfordPetsDataset(os.path.join(basedir,'images'), img_labels=test_labels, transform=data_transforms)

    train_labeled_loader = DataLoader(train_labeled_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)
    train_unlabeled_loader = DataLoader(train_unlabeled_dataset, batch_size=batch_size*split_ratio, shuffle=True, num_workers=num_workers)
    val_labeled_loader = DataLoader(val_labeled_dataset, batch_size=batch_size, shuffle=True, num_workers=num_workers)
...
if __name__ == "__main__":
    ...
    images, mask = next(iter(train_unlabeled_loader))
    print("Unlabeled training images:")
    imshow(images[0], mask[0])

    images, mask = next(iter(val_labeled_loader))
    print("Labeled validation images:")
    imshow(images[0], mask[0])

    images, mask = next(iter(test_labeled_loader))
